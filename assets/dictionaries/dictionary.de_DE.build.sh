...
echo "Removing blacklist words and dumping dictionary to $DICT_FILE..."
grep --line-regexp --invert-match --file="$BUILD_DIR/remove" "$BUILD_DIR/keep" | awk 'length($0) < 10 && length($0) > 2' > "$DICT_FILE"
